...
assert_file "$REPO_ROOT/zsh/config.zsh.example" "config.zsh.example 存在"
...
cat > "$COPY_XDG/zsh/config.zsh" <<'EOLOCAL'
...
typeset -g CONFIG_MARK="loaded_from_config"
EOLOCAL

SMOKE_COPY_L_SCRIPT='
print -r -- "CONFIG_MARK=${CONFIG_MARK:-}"
print -r -- "FEATURES=${ZSH_ENABLE_HISTORY:-}/${ZSH_ENABLE_COMPLETION:-}/${ZSH_ENABLE_KEYBINDS:-}/${ZSH_ENABLE_PROMPT:-}"
print -r -- "KEYMAP=${ZSH_KEYMAP:-}"
print -r -- "THEME=${ZSH_THEME:-}"
print -r -- "LOGIN_STAGE=${__zsh_stage_loaded[login]:-0}"
...
assert_contains "$COPY_L_OUT" "CONFIG_MARK=" "login 阶段不应加载 config.zsh"
...
assert_contains "$COPY_I_OUT" "CONFIG_MARK=loaded_from_config" "interactive 阶段已加载 config.zsh"
assert_contains "$COPY_I_OUT" "FEATURES=0/0/0/0" "config.zsh 覆盖开关成功"
assert_contains "$COPY_I_OUT" "KEYMAP=vi" "config.zsh 覆盖 keymap 成功"
assert_contains "$COPY_I_OUT" "THEME=basic" "config.zsh 覆盖 theme 成功"
